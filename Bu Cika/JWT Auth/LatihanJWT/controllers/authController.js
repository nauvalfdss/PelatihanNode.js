...
// jwtSecret membacanya dari .env.JWT_SECRET
const jwtSecret = process.env.JWT_SECRET;
if (!jwtSecret) throw new Error('JWT_SECRET belum di set');

// membuat 10 karakter acak
...
module.exports = {
  ...
  register: async (req, res, next) => {
    ...
    //   biar dia bisa masuk autoincrement
...
login: async (req, res, next) => {
  try {
    ...
    //else kalau ketemu data user(emai) 
    const user = rows[0];
    ...
    // else berhasil login
    // tokon akan diberikan
    // generate jwt / sign JWT
    // sign jwt (payload: id,email; expiry)
